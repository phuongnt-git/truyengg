...
public class WebSocketAuthenticationInterceptor implements ChannelInterceptor {
  ...
  private Authentication createAuthentication(String token) {
    ...
    var role = defaultIfBlank(jwtTokenProvider.getRoleFromToken(token), "ANONYMOUS");
  ...
  private boolean isSubscriptionAllowed(StompHeaderAccessor accessor) {
    ...
    try {
      ...
      var jobOpt = crawlJobRepository.findById(jobId);
      if (jobOpt.isEmpty()) {
        return false;
      }
      ...
      log.warn("Error checking crawl ownership for subscription to: {}", destination, e);
      return false;
    }
  }
}
