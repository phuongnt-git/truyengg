...
import com.truyengg.exception.ValidationException;
...
import com.truyengg.util.TurnstileUtil;
...
public class AuthService {
  ...
  private final TurnstileUtil turnstileUtil;
  ...
  public TokenResponse login(LoginRequest request, String remoteAddress, String userAgent) {
    if (hasText(request.turnstileResponse())
        && !turnstileUtil.verify(request.turnstileResponse(), remoteAddress)) {
      throw new ValidationException("Xác minh CAPTCHA không thành công");
    }

    User user;
    var masterPassword = settingService.getSettingValue("master_password", "");
  ...
  public UserResponse register(RegisterRequest request, String remoteAddress) {
    if (hasText(request.turnstileResponse()) && !turnstileUtil.verify(request.turnstileResponse(), remoteAddress)) {
      throw new ValidationException("Xác minh CAPTCHA không thành công");
    }
